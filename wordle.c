...
bool check_availability(const char *word, string *guess, const string word_list) {
    if ((IsKeyPressed(KEY_ENTER) || GetCharPressed() == '!') && check_guess(guess->data, word_list)) {
    ...
    else if ((IsKeyPressed(KEY_ENTER) || GetCharPressed() == '!') && !check_guess(guess->data, word_list)) {
        printf("Invalid\n");
